...
class WorkManagerSercice {
  ...
  void regestermytask() async {
    await Workmanager().registerPeriodicTask("1", "callback",
        frequency: const Duration(hours: 23), tag: "lunch notification");
    log("regester lunch task");
  }

  @pragma('vm:entry-point')
  void regesterdinner() async {
    await Workmanager().registerPeriodicTask("2", "dinner notifications",
        frequency: const Duration(hours: 23), tag: "dinner notification");
    log("regester dinner task");
  }

  @pragma('vm:entry-point')
  void watertask() async {
    await Workmanager().registerPeriodicTask("3", "water notifications",
        frequency: const Duration(hours: 4), tag: "water notification");
    log("regester water task");
  }

  @pragma('vm:entry-point')
  void breakfasttask() async {
    await Workmanager().registerPeriodicTask("4", "breakfast notifications",
        frequency: const Duration(hours: 23), tag: "breakfast notification");
